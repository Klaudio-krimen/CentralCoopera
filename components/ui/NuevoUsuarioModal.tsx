...
export default function NuevoUsuarioModal() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', role: 'CHOFER',
  })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 bg-white rounded-2xl shadow-[0_24px_48px_-12px_rgba(0,0,0,0.18)] w-full max-w-md p-6 animate-fade-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-zinc-700">Contraseña temporal</label>
                <input
                  type="password"
                  value={form.password}
                  ...
                  required
                />
                <p className="text-xs text-zinc-400">El usuario deberá cambiarla en el primer ingreso.</p>
              </div>

              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-zinc-700">Rol</label>
                <select
                  value={form.role}
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {loading ? <SpinnerGap size={16} className="animate-spin" /> : 'Crear usuario'}
                </button>
              </div>
            </form>
          </div>
