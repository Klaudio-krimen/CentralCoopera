...
export async function POST(req: NextRequest) {
  ...
  if (contactId && contacts.length === 0) return apiError('Contacto no encontrado', 404)
  ...
  for (const contact of contacts) {
    ...
    const daysSinceLastActivity = Math.floor((now - lastActivityAt.getTime()) / MS_PER_DAY)
    ...
    await prisma.contact.update({ where: { id: contact.id }, data: { score, temperature } })
