...
export async function POST(req: NextRequest) {
  ...
  if (!['ENTRADA', 'SALIDA', 'AJUSTE'].includes(type)) return apiError('Tipo de movimiento inválido')
  ...
  if (type === 'ENTRADA') newQuantity = item.quantity + qty
    ...
    if (newQuantity < 0) return apiError('No hay stock suficiente para esta salida')
  } else {
