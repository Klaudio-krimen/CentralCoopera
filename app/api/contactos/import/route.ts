...
const PLACEHOLDER_VALUES = new Set(['sin dato', 'sindato', 'n/a', 'na', '-', 'null', 'undefined'])
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (rating) extraLines.push(`Rating: ${rating}${reviews ? ` (${reviews} reseñas)` : ''}`)
      if (qualification) extraLines.push(`Señal de calificación: ${qualification}`)
      if (date) extraLines.push(`Fecha de origen: ${date}`)

      const notes = [clean(row.notes), extraLines.length > 0 ? extraLines.join('\n') : undefined]
        .filter(Boolean)
        .join('\n\n')
