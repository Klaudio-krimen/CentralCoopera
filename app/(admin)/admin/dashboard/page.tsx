...
async function getDashboardData() {
  ...
  const [totalMes, activas, discrepancias, recentOrders] = await Promise.all([
    ...
    prisma.pickupOrder.count({
      where: { status: { in: ["EN_RETIRO", "EN_TRANSITO"] } },
    }),
...
export default async function AdminDashboardPage() {
  const { totalMes, activas, discrepancias, recentOrders } =
          ...
          const body = (
            <div className="p-5 sm:p-6 h-full">
              ...
              <div className="flex items-end justify-between">
                <p
                  className={`text-[2.5rem] font-semibold tracking-tight leading-none font-mono tabular-nums ${tone}`}
                >
                  {value}
                </p>
                {href && (
          ...
          {recentOrders.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-zinc-500 mt-0.5">
                Las órdenes en curso aparecerán aquí
              </p>
            </div>
          ) : (
            <div className="divide-y divide-zinc-100">
              {recentOrders.map((order) => (
                ...
                >
                  <p className="text-xs font-mono text-zinc-400 w-28 shrink-0 tabular-nums">
                    {order.orderCode}
                  </p>
