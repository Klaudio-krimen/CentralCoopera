...
export default function KanbanBoard({ initialColumns }: { initialColumns: PipelineColumn[] }) {
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    const activeColumn = columns.find((c) => c.deals.some((d) => d.id === activeId))
    const overColumn =
    ...
    setColumns((prev) => {
      ...
      if (!deal) return prev
      return prev.map((c) => {
        if (c.id === activeColumn.id) return { ...c, deals: c.deals.filter((d) => d.id !== activeId) }
        if (c.id === overColumn.id) return { ...c, deals: [...c.deals, deal] }
        return c
      })
    })
  }, [columns])
    ...
    <div className="space-y-3">
      ...
      >
        <div className="flex gap-3 overflow-x-auto pb-4">
          {columns.map((col) => (
            <KanbanColumn key={col.id} id={col.id} name={col.name} color={col.color} deals={col.deals} />
          ))}
        </div>
